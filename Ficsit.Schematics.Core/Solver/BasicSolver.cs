...
public sealed class BasicSolver(GameDatabase data) : ISolver
{
    ...
    {
        // Flat model: solve every node except the outpost/blueprint containers (pure
        // groupings, not machines). Connections are real node→node and cross outpost
        // boundaries freely; any stray connection touching a container is skipped defensively.
        var nodes = document.Root.AllNodes()
            .Where(n => n.Kind is not (NodeKind.Outpost or NodeKind.Blueprint))
            .ToList();
        var nodeSet = nodes.ToHashSet();
        var connections = document.Root.AllConnections()
            .Where(c => nodeSet.Contains(c.From) && nodeSet.Contains(c.To))
            .ToList();
