...
public sealed partial class FactoryCanvasDrawable
{
    ...
    {
        // Edge rail while dragging a wire inside an outpost: release here to wire the port to
        // the outer world (left = fed from outside, right = sent out).
        if (EdgeDropZone is { } left)
        {
            ...
            canvas.DrawString(left ? "◀ from outside" : "to outside ▶", labelRect,
