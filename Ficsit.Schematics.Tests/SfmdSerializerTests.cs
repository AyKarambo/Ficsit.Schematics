...
public class SfmdSerializerTests
{
    ...
    [Theory]
    [InlineData(LogisticsKind.Truck)]
    [InlineData(LogisticsKind.Drone)]
    [InlineData(LogisticsKind.Train)]
    public void Roundtrip_preserves_a_connections_logistics_kind(LogisticsKind kind)
    {
        ...
        doc.Root.Connections.Add(new NodeConnection
            { From = producer, To = consumer, Part = "Iron Ingot", Logistics = kind });
        ...
        Assert.Equal(kind, connection.Logistics);
