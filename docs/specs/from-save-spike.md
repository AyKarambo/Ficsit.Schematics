...
> **Status: ✅ Implemented and extended** (the spike's research, below, is now built — and broadened to the *whole* progression, not just alternates). `SatisfactorySaveReader.ReadUnlockedSchematics` extracts every purchased schematic stem (milestones "P-M", tutorials, MAM, hard-drive `Alternate_*`) by walking the array's `/Game/.../Schematics/` paths. `SchematicRecipeMap.DetermineUnlocked` then computes the enabled recipe set: **standard recipes gated by the highest reached phase** (milestone schematics), **alternates gated by their specific hard-drive** (token-set matcher + a catalog-validated override table). Both the recipe list's **"From save"** button and the Settings **Import save** flow apply it, disabling every recipe the save hasn't unlocked. (MAM-gated recipes don't map to a milestone, so the phase gate keeps them on within the reached tier rather than wrongly disabling them — a documented approximation.) Implementation note: the spike's "add `UnlockSchematic` to `game_data.json`" idea predates the grouped-C# catalog and was superseded by the token-set matcher + tier gate.
